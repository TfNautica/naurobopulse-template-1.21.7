package ntc.tfnautica.naurobopulse.render;
...
public class renderCustomOverlay implements HudElement {
    ...
    private static renderCustomOverlay hudInstance;

    private renderCustomOverlay() {
    }

    public static renderCustomOverlay getInstance() {
        if(hudInstance == null) {
            hudInstance = new renderCustomOverlay();
        }
        return hudInstance;
    }
    @Override
    public void render(DrawContext context, RenderTickCounter renderTickCounter) {
        ...
        String timeString = String.format("%02d:%02d", new Object[] { Integer.valueOf(hours), Integer.valueOf(minutes) });
